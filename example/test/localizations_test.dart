...
void main() {
  group('AppLocalizations Unit Tests', () {
    ...
    test('Auto language detection works', () {
      ...
      expect(autoLocalizations.languageCode, isIn(AppLocalizations.supportedLanguages));
    });
    ...
    test('Different language instances produce different translations', () {
      if (AppLocalizations.supportedLanguages.length > 1) {
        ...
        final secondLang = AppLocalizations(AppLocalizations.supportedLanguages[1]);
  ...
  group('CamelCase Method Name Tests', () {
    test('Snake_case keys are converted to camelCase methods', () {
      ...
      expect(() => loc.login.welcomeMessage(name: 'test'), returnsNormally); // welcome_message
      expect(() => loc.login.userCount(count: 1), returnsNormally);     // user_count
