...
void main() {
  ...
  group('Language Code Validation Tests', () {
    ...
    test('should reject invalid language codes', () {
      ...
      expect(ExcelLocalizationsGenerator.isValidLanguageCode('invalid'), isFalse);
  ...
  group('Sheet Name Sanitization Tests', () {
    test('should sanitize sheet names correctly', () {
      expect(ExcelLocalizationsGenerator.sanitizeSheetName('Login Page'), equals('loginpage'));
      expect(ExcelLocalizationsGenerator.sanitizeSheetName('User-Settings'), equals('usersettings'));
      expect(ExcelLocalizationsGenerator.sanitizeSheetName('123Numbers'), equals('sheet\$023numbers'));
      expect(ExcelLocalizationsGenerator.sanitizeSheetName('Special@#Characters'), equals('specialcharacters'));
    });
  ...
  group('Method Name Sanitization Tests', () {
    test('should convert snake_case to camelCase', () {
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('app_title'), equals('appTitle'));
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('welcome_message'), equals('welcomeMessage'));
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('user_count'), equals('userCount'));
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('save_button'), equals('saveButton'));
    });

    test('should handle single words', () {
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('hello'), equals('hello'));
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('goodbye'), equals('goodbye'));
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('submit'), equals('submit'));
    });

    test('should handle special characters', () {
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('app-title'), equals('apptitle'));
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('user@name'), equals('username'));
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('test#key'), equals('testkey'));
    });

    test('should handle numbers at start', () {
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('123test'), equals('123test')); // Single word, no underscore, no key prefix
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('1_title'), equals('key\$0Title')); // Has underscore, gets processed and key prefix
    });

    test('should handle empty underscore parts', () {
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('app__title'), equals('appTitle'));
      expect(ExcelLocalizationsGenerator.sanitizeMethodName('test___key'), equals('testKey'));
    });
  });

  group('String Interpolation Tests', () {
    test('should extract interpolation parameters', () {
      final params1 = ExcelLocalizationsGenerator.extractInterpolationParams('Welcome {name}!');
      expect(params1, equals(['name']));

      final params2 = ExcelLocalizationsGenerator.extractInterpolationParams('You have {count} items in {location}');
      expect(params2, containsAll(['count', 'location']));
      expect(params2.length, equals(2));
    });

    test('should handle no interpolation', () {
      final params = ExcelLocalizationsGenerator.extractInterpolationParams('Simple message');
      expect(params, isEmpty);
    });

    test('should handle duplicate parameters', () {
      final params = ExcelLocalizationsGenerator.extractInterpolationParams('Hello {name}, welcome {name}!');
      expect(params, equals(['name'])); // Should deduplicate
    });

    test('should handle complex parameters', () {
      final params = ExcelLocalizationsGenerator.extractInterpolationParams('User {user_id} has {item_count} items');
  ...
  group('Language Name Resolution Tests', () {
    test('should resolve known language names', () {
      expect(ExcelLocalizationsGenerator.getLanguageName('en'), equals('english'));
      expect(ExcelLocalizationsGenerator.getLanguageName('id'), equals('indonesian'));
      expect(ExcelLocalizationsGenerator.getLanguageName('es'), equals('spanish'));
    });
    ...
    test('should fallback to code for unknown languages', () {
      expect(ExcelLocalizationsGenerator.getLanguageName('unknown'), equals('unknown'));
  ...
  group('Language Code Validation Edge Cases', () {
    ...
    test('should validate complex locale formats', () {
      expect(ExcelLocalizationsGenerator.isValidLanguageCode('en_US_POSIX'), isFalse); // Too complex for current implementation
      expect(ExcelLocalizationsGenerator.isValidLanguageCode('en_'), isTrue); // Current implementation accepts this
      expect(ExcelLocalizationsGenerator.isValidLanguageCode('_US'), isFalse); // No language code before underscore
    });
  });
  ...
  group('Error Handling Tests', () {
    ...
    test('should provide helpful error messages for invalid language codes', () {
      expect(
        () => ExcelLocalizationsGenerator.validateLanguageCodes(['invalid', 'xyz'], 'TestSheet'),
        throwsA(predicate((e) => e.toString().contains('Invalid language codes'))),
      );
    });
  });
}
